...
void CallGraphPass::findCalleesWithType(CallInst *CI, FuncSet &S) {
	...
	for (Function *F : Ctx->AddressTakenFuncs) {
		...
		bool Matched = true;
		CallBase::op_iterator AI = CI->arg_begin();
		for (Function::arg_iterator FI = F->arg_begin(),
			...
			if (DefinedTy->isStructTy() && ActualTy->isStructTy() &&
                            (DefinedTy->getStructName().
                             equals(ActualTy->getStructName())))
				continue;
			if (DefinedTy->isIntegerTy() && ActualTy->isIntegerTy() &&
                            DefinedTy->getIntegerBitWidth() ==
                            ActualTy->getIntegerBitWidth())
				continue;
...
bool CallGraphPass::typeConfineInStore(StoreInst *SI) {
  ...
  // Use getPointerElementType for LLVM 15 and newer
  Type *EPTy = VO->getType();
  Type *VTy = VO->getType();
  ...
  if (nextLayerBaseType(PO, STy, Idx, DL)) {
    ...
    if (isCompositeType(EVTy)) {
      typeConfineMap[typeIdxHash(STy, Idx)].
        insert(typeHash(EVTy));
      return true;
    }
    else {
...
void CallGraphPass::transitType(Type *ToTy, Type *FromTy,
  ...
  if (ToIdx != -1 && FromIdx != -1)
    typeTransitMap[typeIdxHash(ToTy, ToIdx)].
      insert(typeIdxHash(FromTy, FromIdx));
  else
    typeTransitMap[typeHash(ToTy)].insert(typeHash(FromTy));
}

void CallGraphPass::
funcSetIntersection(FuncSet &FS1, FuncSet &FS2,
                    FuncSet &FS) {
  FS.clear();
  for (auto F : FS1) {
...
// memory loads
Value *CallGraphPass::
nextLayerBaseType(Value *V, Type * &BTy,
    ...
    for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; ++i) {
      ...
      if (CallInst *CI = dyn_cast<CallInst>(&*i)) {
        ...
        if (CI->isIndirectCall()) {
          // skip for now, since llvm 15 change type a lot
          /*
