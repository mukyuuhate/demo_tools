...
int main(int argc, char **argv){
  ...
  FindErrorFuncsPass fefpass(&GlobalCtx);
  fefpass.run(GlobalCtx.Modules);
  fefpass.dump_func_use_map();
  // fefpass.read_source_code("/home/mukyuuhate/Documents/trusty_bc/all.part1.lst");

  // std::string input = "error_message";
  // bool isMatch = fefpass.is_error_func(input);
  // std::cout << std::boolalpha << isMatch << std::endl;

  // input = "critical_alert";
  // isMatch = fefpass.is_error_func(input);
  // std::cout << std::boolalpha << isMatch << std::endl;
